...
**Send to cover log** writes the request to the shared
[cover log](#global-cover-log). **Copy email** puts it on the clipboard instead.

If you see *This timetable is not linked to a workspace, so there is no cover
log to write to*, the session is not part of a global workspace — see
[Linking sessions](#global-linking-sessions).
