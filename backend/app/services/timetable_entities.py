...
from .global_staff_hours import staff_tab_total_hours_map_for_session
...
def _staff_entities(db: Session, timetable_session_id: int) -> list[dict]:
    ...
    hours_map = staff_tab_total_hours_map_for_session(
        db, timetable_session_id, staff_rows=staff
    )
    rows: list[dict] = []
    for s in staff:
