...
def import_asc_xml_export(
    ...
    *,
    ...
    for lid, elem in lessons_by_id.items():
        ...
        length = _parse_length(_attr(elem, "periodspercard"))
        ...
        teacher_ids = _split_ids(_attr(elem, "teacherids"))
        if teacher_ids:
        ...
        for room in _rooms_for_classroom_ids(
            ...
            if not session.query(UnitAllowedRoom).filter_by(
    ...
    if cards:
        ...
        grouped: dict[tuple[str, str], list[_XmlCard]] = defaultdict(list)
        for card in cards:
            grouped[(card.lesson_id, card.days)].append(card)

        course_unit_linked: set[tuple[int, int]] = set()
        for (lesson_id, days), card_rows in grouped.items():
            ...
            unit = unit_cache.get((qual_key, _norm(subject_code)))
            if course is None or unit is None:
                rep.warnings.append(
                    f"Cards for lesson {lesson_id}: no course/unit for {subject_code!r}/{class_name!r}"
                )
                continue
