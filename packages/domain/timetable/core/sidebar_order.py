...
def ordered_courses(session: Session, *, include_block_cohorts: bool = False) -> list[Course]:
...
def ordered_staff(session: Session) -> list[Staff]:
    return (
        session.query(Staff)
        .order_by(Staff.sidebar_order, Staff.name, Staff.id)
        .all()
    )
